...
\begin{tikzpicture}[
  ...
  stage/.style={draw=navy,fill=paperblue,rounded corners=2.2pt,
    line width=.7pt,text width=3.72cm,minimum height=1.22cm,
    align=left,inner xsep=6mm,inner ysep=3mm,font=\sffamily\small},
  ...
  finitebox/.style={draw=finite!85!black,fill=finite!5,rounded corners=2pt,
    line width=.7pt,text width=3.45cm,minimum height=.72cm,
    align=center,inner sep=2mm,font=\sffamily\scriptsize},
  badge/.style={circle,fill=navy,text=white,inner sep=0pt,
    minimum size=5.6mm,font=\sffamily\bfseries\scriptsize},
  redbadge/.style={badge,fill=open},
...
\node[note,anchor=west] at (-7.7,2.0) {ALGEBRAIC REDUCTION};
\node[stage] (s1) at (-5.25,1.15)
  {\textbf{Recast the shape}\\[-1pt]\scriptsize Reciprocal $\Rightarrow$ identity ray class};
\node[stage] (s2) at (0,1.15)
  {\textbf{Count prime mass}\\[-1pt]\scriptsize Log derivative + M\"obius};
\node[stage] (s3) at (5.25,1.15)
  {\textbf{Decompose exactly}\\[-1pt]\scriptsize Haar split + exact Witt orders};
\node[badge] at ($(s1.north west)+(0.34,-0.34)$) {1};
\node[badge] at ($(s2.north west)+(0.34,-0.34)$) {2};
\node[badge] at ($(s3.north west)+(0.34,-0.34)$) {3};

\node[note,anchor=west] at (-7.7,-.05) {ANALYTIC FRONTIER};
\node[stage] (s6) at (-5.25,-1.0)
  {\textbf{Extract the prime}\\[-1pt]\scriptsize Mass beats proper prime powers};
\node[frontier] (s5) at (0,-1.0)
  {\textbf{High-Witt cancellation}\\[-1pt]\scriptsize Connected HWO / NSD / REL};
\node[stage] (s4) at (5.25,-1.0)
  {\textbf{Pay the known range}\\[-1pt]\scriptsize Weil bounds + wrapped energy};
\node[badge] at ($(s6.north west)+(0.34,-0.34)$) {6};
\node[redbadge] at ($(s5.north west)+(0.34,-0.34)$) {5};
\node[badge] at ($(s4.north west)+(0.34,-0.34)$) {4};
...
\node[finitebox] (cert) at (-5.25,-2.15)
  {\textbf{Finite handoff}\quad Rabin certificates for $n\leq400$};
